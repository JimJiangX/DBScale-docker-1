...
ResponsiveDatatablesHelper.prototype.init = function (a, b) {
    ...
    if (this.saveState && this.getState(), !this.lastStateExists) {
        ...
        for (d = 0, e = f.length; e > d; d++)this.api.column(d).visible() && (this.columnIndexes.push(d), g.push(f[d]));
    ...
    if (!this.disabled) {
        ...
        })) : (this.tableElement.removeClass("has-columns-hidden"), $("tr.row-detail", this.tableElement).each(function (b) {
    ...
    if (a + b > 0) {
        ...
        localStorage.setItem(this.cookieName, JSON.stringify(c)), this.lastColumnsHiddenIndexes = this.columnsHiddenIndexes.slice(0)
    }
