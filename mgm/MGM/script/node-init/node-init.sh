...
swarm_agent_version=1.2.8-91825aa
...
rpm_install() {
	...
	elif [ "${release}" == "RedHatEnterpriseServer" ] || [ "${release}" == "CentOS" ] || [ "${release}" == "UnionPayEnterprise" ]; then
...
install_docker() {
	...
	do
		link_status=$(cat /sys/class/fc_host/${fc_host}/port_state)
		if [ "${link_status}" == "Online" ]; then
	...
	if [ "${release}" == "RedHatEnterpriseServer" ] || [ "${release}" == "CentOS" ] || [ "${release}" == "UnionPayEnterprise" ]; then
		if [ "${san_id}" != '' ]; then
...
install_swarm_agent() {
	...
	chmod +x ${script_dir}/seed/net/* ${script_dir}/seed/san/* ${script_dir}/*.sh
